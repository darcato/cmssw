...
namespace pixelgpudetails {
  ...
  struct fillHitsModuleStart {
    ...
    template <typename T_Acc>
    ALPAKA_FN_ACC
    void operator()(T_Acc const& acc, uint32_t const *__restrict__ cluStart, uint32_t *__restrict__ moduleStart) const {
  ...
  void SiPixelRawToClusterGPUKernel::makeClustersAsync(const SiPixelFedCablingMapGPU *cablingMap,
    ...
    if (wordCounter)  // protect in case of empty event....
    {
      ...
      cudaCheck(cudaMemcpyAsync(
            word_d.get(), wordFed.word(), wordCounter * sizeof(uint32_t), cuplaMemcpyHostToDevice, stream.id()));
      cudaCheck(cudaMemcpyAsync(
            fedId_d.get(), wordFed.fedId(), wordCounter * sizeof(uint8_t) / 2, cuplaMemcpyHostToDevice, stream.id()));

      // Launch rawToDigi kernel
      CUPLA_KERNEL(RawToDigi_kernel)(blocks, threadsPerBlock, 0, stream.id())(
          cablingMap,
          modToUnp,
          wordCounter,
          word_d.get(),
...

    {
      ...
      CUPLA_KERNEL(gpuCalibPixel::calibDigis)(blocks, threadsPerBlock, 0, stream.id())(digis_d.moduleInd(),
          digis_d.c_xx(),
          digis_d.c_yy(),
          digis_d.adc(),
          gains,
      ...
      CUPLA_KERNEL(countModules)(blocks, threadsPerBlock, 0, stream.id())(
          digis_d.c_moduleInd(), clusters_d.moduleStart(), digis_d.clus(), wordCounter);
      cudaCheck(cudaGetLastError());

      // read the number of modules into a data member, used by getProduct())
      cudaCheck(cudaMemcpyAsync(
          &(nModules_Clusters_h[0]), clusters_d.moduleStart(), sizeof(uint32_t), cuplaMemcpyDeviceToHost, stream.id()));
...
#endif
      CUPLA_KERNEL(findClus)(blocks, threadsPerBlock, 0, stream.id())(digis_d.c_moduleInd(),
          digis_d.c_xx(),
          digis_d.c_yy(),
      ...
      // apply charge cut
      CUPLA_KERNEL(clusterChargeCut)(blocks, threadsPerBlock, 0, stream.id())(digis_d.moduleInd(),
          digis_d.c_adc(),
          clusters_d.c_moduleStart(),
          clusters_d.clusInModule(),
      ...
      // MUST be ONE block
      CUPLA_KERNEL(fillHitsModuleStart)(1, 1024, 0, stream.id())(clusters_d.c_clusInModule(), clusters_d.clusModuleStart());
